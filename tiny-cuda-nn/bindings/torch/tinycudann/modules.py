...
# before: _rtc_cache_dir = os.path.join(os.path.dirname(__file__), "rtc", "cache")
_env_cache = os.environ.get("TCNN_RTC_CACHE_DIR")
if _env_cache:
    _rtc_cache_dir = os.path.expanduser(_env_cache)
else:
    _rtc_cache_dir = os.path.join(os.path.dirname(__file__), "rtc", "cache")

# ensure the directory exists (in a writable place)
try:
    os.makedirs(_rtc_cache_dir, exist_ok=True)
except Exception as e:
    # optional: log warning but continue
    import warnings
    warnings.warn(f"tinycudann: could not create rtc cache dir {_rtc_cache_dir}: {e}")
os.makedirs(_rtc_cache_dir, exist_ok=True)
