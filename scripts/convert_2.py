...
def main(colmap_sparse_dir, output_json, image_dir=None):
    ...
    # Step 2: build camera_track_map and get camera centers
    ...
    for idx, image_id in enumerate(sorted(image_order)):
        ...
        extrinsic_flat = make_extrinsic(img_info['qvec'], img_info['tvec'])
    ...
    radius = np.max(np.linalg.norm(cam_centers - center, axis=1))

    # Create a scaling and translation matrix to map the scene to a unit cube
    scale_mat = np.eye(4)
    scale_factor = 1.0 / radius
    scale_mat[0, 0] = scale_mat[1, 1] = scale_mat[2, 2] = scale_factor

    translation_mat = np.eye(4)
    translation_mat[:3, 3] = -center

    transform_matrix = scale_mat @ translation_mat

    bbox = {
        "transform": transform_matrix.flatten().tolist()
    }
